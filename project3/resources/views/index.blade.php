                ...
                <form action="{{ route('add.to.cart', $product->id) }}" method="GET">
                    <div class="pull-right price" style="font-weight: Bold; margin-bottom: 10px;">
                        ...
                        <select name="size" id="size">
                            <option value="small">Small - €{{ number_format($product->getPriceBySize('small'), 2) }}
                            </option>
                            <option value="medium">Medium - €{{ number_format($product->getPriceBySize('medium'), 2) }}
                            </option>
                            <option value="large">Large - €{{ number_format($product->getPriceBySize('large'), 2) }}
                            </option>
                        </select>
                    </div>
                    @csrf
